...
__global__ void computeReflectanceKernel(RealMatrix reflectance, Field E_inc, Field E_scat, int n) {
    ...
    if (i < n) {
        ...
        double denominator = getSquaredNorm(E_scat.x.getDeviceRealValue(i), E_scat.x.getDeviceImagValue(i),
                                            E_scat.y.getDeviceRealValue(i), E_scat.y.getDeviceImagValue(i),
                                            E_scat.z.getDeviceRealValue(i), E_scat.z.getDeviceImagValue(i));
...
void BioScat::computeReflectance() {
    ...
    if (deviceComputation) {
        ...
        computeReflectanceKernel<<<dimGrid, dimBlock>>>(reflectance, E_inc, E_scat, n);
        ...
        for (int i = 0; i < x_obs.rows; i++) {
            ...
            double denominator = getSquaredNorm(E_scat.x.getHostRealValue(i), E_scat.x.getHostImagValue(i),
                                                E_scat.y.getHostRealValue(i), E_scat.y.getHostImagValue(i),
